# 🍽️ SMART RESTAURANT MANAGEMENT SYSTEM

A web-based Restaurant Management System developed using **Python,
Flask, HTML, CSS, JavaScript, and SQLite**. The application streamlines
restaurant operations by allowing customers to browse menus, place
orders, search food items, make payments, and generate digital receipts.

------------------------------------------------------------------------

# 📑 Table of Contents

-   📌 Project Overview
-   🎯 Objectives
-   🛠️ Technologies Used
-   🏗️ System Architecture
-   🍴 Restaurant Modules
-   🍽️ Menu Categories
-   ⚙️ Project Workflow
-   📂 Project Structure
-   💻 Installation
-   ▶️ Running the Project
-   ✨ Features
-   🌍 Applications
-   📷 Project Screenshots
-   📊 Results
-   🔮 Future Enhancements
-   👨‍💻 Author
-   📄 License

------------------------------------------------------------------------

# 📌 Project Overview

## Overview

The Smart Restaurant Management System is a Flask-based web application
that digitizes restaurant operations. It enables customers to browse
menus, place orders, search dishes, make payments, and receive digital
receipts. SQLite is used for data storage, while HTML, CSS, and
JavaScript provide the user interface.

# 🎯 Objectives

-   Automate restaurant operations.
-   Provide an easy-to-use ordering system.
-   Reduce manual effort.
-   Support digital payments.
-   Store restaurant data securely.
-   Improve customer experience.

# 🛠️ Technologies Used

## Programming Language

-   Python

## Backend Framework

-   Flask

## Frontend Technologies

-   HTML5
-   CSS3
-   JavaScript

## Database

-   SQLite

## Payment Integration

-   UPI
-   Razorpay (if configured)

## Development Tools

-   Visual Studio Code
-   Git
-   GitHub

# 🏗️ System Architecture

## Presentation Layer

Handles the web interface for customers.

## Business Logic Layer

Processes orders, payments, search, and receipt generation using Flask.

## Database Layer

Stores menu, customer, and order information in SQLite.

# 🍴 Restaurant Modules

## Customer Module

Browse menu, search items, place orders.

## Menu Management

Display categorized menu items.

## Order Management

Create and manage customer orders.

## Payment Module

Handle payment options and confirmation.

## Receipt Generation

Generate digital receipts after successful payment.

## Search Module

Search menu items quickly.

# 🍽️ Menu Categories

-   Veg Menu
-   Non-Veg Menu
-   South Indian
-   North Indian
-   Starters
-   Cakes
-   Drinks
-   Special Items

# ⚙️ Project Workflow

Home Page → Browse Menu → Select Items → Place Order → Payment → Receipt

# 📂 Project Structure

``` text
SMART-RESTAURANT-MANAGEMENT-SYSTEM/
├── CSS/
├── database/
│   ├── create_db.py
│   ├── restaurant.db
│   └── view_db.py
├── Flask/
│   └── app.py
├── HTML/
├── README.md
└── LICENSE
```

# 💻 Installation

``` bash
git clone https://github.com/your-username/SMART-RESTAURANT-MANAGEMENT-SYSTEM.git
cd SMART-RESTAURANT-MANAGEMENT-SYSTEM
python -m venv venv
```

Install dependencies:

``` bash
pip install flask
```

# ▶️ Running the Project

``` bash
python Flask/app.py
```

Open:

    http://127.0.0.1:5000

# ✨ Features

-   Restaurant homepage
-   Food menu browsing
-   Category-wise menu
-   Food search
-   Online ordering
-   SQLite database
-   Digital payment pages
-   Receipt generation
-   Responsive interface

# 🌍 Applications

-   Restaurants
-   Cafeterias
-   Food courts
-   College canteens
-   Small food businesses

# 📷 Project Screenshots

Include screenshots of: - Home Page - Menu - Order Page - Payment Page -
Receipt - Search

# 📊 Results

-   Simplified restaurant ordering.
-   Reduced manual work.
-   Improved customer experience.
-   Organized menu and order management.

# 🔮 Future Enhancements

-   Admin Dashboard
-   Inventory Management
-   Employee Management
-   Online Table Reservation
-   Customer Login
-   AI-based Food Recommendation
-   Analytics Dashboard
-   Docker Deployment
-   Cloud Deployment
-   Mobile Application

# 👨‍💻 Author

**Sathish Reddy**

B.Tech CSE Student \| Python & Flask Developer

# 📄 License

This project is licensed under the MIT License.
